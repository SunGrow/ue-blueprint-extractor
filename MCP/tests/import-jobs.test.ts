...
describe('import tool aliases', () => {
  beforeEach(() => {
    aliasMap.clear();
    rawHandlerRegistry.clear();
  });

  it('import_textures alias delegates to import_assets with texture_options mapping', async () => {
    const callSubsystemJson = vi.fn(async () => ({
      success: true,
      jobId: 'tex-alias-1',
    }));
    ...
    try {
        ...
        arguments: {
          payload: {
            items: [{
              file_path: 'C:/Art/Icon.png',
              destination_path: '/Game/UI',
              options: { compression_settings: 'UserInterface2D' },
            }],
          },
          validate_only: false,
        },
      ...
      expect(result.isError).toBeFalsy();
      // The alias maps options -> texture_options, then the handler maps texture_options -> options for ImportTextures
      expect(callSubsystemJson).toHaveBeenCalledWith('ImportTextures', {
        PayloadJson: JSON.stringify({
          items: [{
            file_path: 'C:/Art/Icon.png',
  ...
  it('import_meshes alias delegates to import_assets with mesh_options mapping', async () => {
    const callSubsystemJson = vi.fn(async () => ({
      success: true,
      jobId: 'mesh-alias-1',
    }));
    ...
    try {
      const result = await harness.client.callTool({
        ...
        arguments: {
          payload: {
            items: [{
              file_path: 'C:/Models/Hero.fbx',
              destination_path: '/Game/Meshes',
              options: { mesh_type: 'SkeletalMesh' },
            }],
          },
          validate_only: true,
        },
  ...
  it('reimport_assets alias delegates to import_assets with reimport=true', async () => {
    const callSubsystemJson = vi.fn(async () => ({
      success: true,
      jobId: 'reimport-alias-1',
    }));
    const { server, toolHelpRegistry } = createFullServerHarness();
  ...
  it('aliasMap contains mappings for all three deprecated import tools', () => {
    const { server, toolHelpRegistry } = createFullServerHarness();

    registerImportJobTools({
      server,
      callSubsystemJson: vi.fn(async () => ({})),
      importPayloadSchema,
      importJobSchema,
      importJobListSchema,
      textureImportPayloadSchema,
    ...
    expect(aliasMap.get('import_textures')).toBe('import_assets');
    expect(aliasMap.get('import_meshes')).toBe('import_assets');
    expect(aliasMap.get('reimport_assets')).toBe('import_assets');
  });
});
