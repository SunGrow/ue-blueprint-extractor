...
| `modify_material` as the default public material workflow | Supported, but secondary | `material_graph_operation` with `set_material_settings`, `add_expression`, `connect_expressions`, and `connect_material_property` | Large batch payloads were harder for models to author and debug |
...
| Literal embedded `AActor` instances inside DataAssets | Unsupported surface | Inline instanced `UObject` graphs through `create_data_asset` / `modify_data_asset` | World-owned actors are not inline asset-owned config data |

## Notes

- `extract_asset` with `asset_type: "data_asset"` still works for Enhanced Input assets because extraction is reflection-safe.
- Generic DataAsset reflection now supports nested inline instanced `UObject` graphs for `UPROPERTY(Instanced)` / `EditInlineNew` values.
